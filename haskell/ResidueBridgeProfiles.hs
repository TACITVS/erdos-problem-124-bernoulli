...
powersUpTo ds exponentStart limit =
  sort (concatMap (\base -> powersFor base exponentStart limit) ds)
...
firstPowersAbove ds exponentStart limit =
  map (\base -> firstPowerAbove base exponentStart limit) ds

exactCriticalDenominator :: [Integer] -> Maybe Integer
exactCriticalDenominator ds =
  let denominator = foldr lcm 1 (map (\base -> base - 1) ds)
      weightSum = sum [denominator `div` (base - 1) | base <- ds]
...
verifyProfile profile = do
  ...
  let finalResidues = last (residueStates (modulus profile) seed)
