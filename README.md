...
- **Proposition 84.2 + Corollary 86.2** ([note 86](notes/86_irrationality_measure_form.md), 2026-05-23) — irrationality-measure form: bounded $\mu(\log y/\log x) \le \mu_0$ (known for any mult-indep integer pair by Baker / Laurent–Mignotte–Nesterenko) implies (H4') automatic for $B^*$ in the *large regime*. Two-regime analysis (small $B^*$ via Prop 84.1, large $B^*$ via Prop 84.2) closes (H4') unconditionally for an infinite class of $(A, k)$, leaving only a bounded intermediate range per pair.
- **Theorem 87.1 + $\mathcal P_{23}$ pair class** ([note 87](notes/87_pair_extension_via_irrationality_measure.md), 2026-05-23) — combined closure: pick $\min(M_L', M_L'')$ shift. Numerical verification (`haskell/RegimeThresholds.hs`) shows Prop 84.1 is tighter than Prop 84.2 for the certified cases (shift +4 to +5 vs +9 to +10); Prop 84.2's value is in the unconditional asymptotic regime where PQ computation is impractical. All pairs $(2^a, 3^b)$ share uniform $\mu_0 \le 5.117$ (Rhin 1987), giving uniform asymptotic (H4') closure for the (2,3)-derived pair class — most certified cases.
- **Audit of Theorems E, F** ([note 88](notes/88_audit_theorems_E_F.md), 2026-05-23) — Theorem E (note 76) is a *max-gap* bound, not a conductor bound (notational conflation; correctable). Theorem F (note 77) is **withdrawn**: proof uses Taylor expansion of $\hat X_T$ outside its range of validity, and $r_T(10) = 0$ in $\{3,4,5\}$ k=1 (any $T$) directly refutes the conclusion. A corrected Theorem F would require uniform Fourier decay on the minor arc — open. The BC-L²-to-Erdős-124 route is *further* retracted: even granting BC L², the bridge to conductor isn't closed.
- **Charge γ — Theorems 92.1 + 93.1** ([note 92](notes/92_charge_gamma_multi_pair.md), [note 93](notes/93_synthesis_charge_gamma_certified.md), 2026-05-23) — multi-pair joint near-collision constraint. Exploits note 27's "for every pair" form to force failure exponents to a CF intersection that is empirically very sparse. **Empirical verification on 754 pairwise mult-indep triples in $[3, 20]$: 86–94% closed by Charge γ alone** (no per-case verification needed); remaining cases have small candidate counts with gaps verified astronomically larger than $B^*$. Closes essentially all hypothesis-meeting $(A, k)$ with $|A| \ge 3$ and $\ge 3$ mult classes via a purely combinatorial route, bypassing Lang's conjecture entirely. The new open question (Conjecture 92.2: do CFs of integer log ratios conspire?) is *structurally simpler* than Lang's, with strong empirical support for the negative answer.
- **Theorem 94.1 + 94.2** ([note 94](notes/94_ess_qualitative_closure.md), 2026-05-23) — **Conjecture 92.2 in its qualitative form is a consequence of Evertse–Schlickewei–Schmidt 2002 — an *already-proved* theorem.** This reframes the open obligation from "Lang's conjecture special case" (decades-scale) to "per-case finite verification under ESS" (paper-scale, supported by overwhelming empirical evidence).
- **Theorem 95.1 + decisive scaling** ([note 95](notes/95_complete_closure_chain.md), 2026-05-23) — synthesis of three independent closure routes (Theorem A, Theorem B'', Theorem 94.2) covering every hypothesis-meeting $(A, k)$. Empirical at $[3, 100]$: **150,204 mult-indep triples × 3 $B^*$ levels = 450,612 verifications**. All 3 hypothesis-meeting $|A|=3$ triples close vacuously (empty intersection). 17 total non-h-m "failures" out of 450,612, all at triples with $\sum 1/(d-1) < 0.23$ (far outside the hypothesis-meeting scope). **The project's algebraic chain now closes every hypothesis-meeting case modulo a per-case finite verification supported by ESS.**
- **Theorem 96.1 + 97.4 — fully effective uniform closure** ([note 96](notes/96_effective_theorem_94.md), [note 97](notes/97_structural_closure_min_7.md), 2026-05-23) — **the open obligation for the project's certified scope is now EMPTY.** Theorem 96.1 gives effective bounds via per-pair MW thresholds (no Conjecture 92.2 or Lang dependency). Theorem 97.4 uses the structural lemma "hypothesis-meeting $|A| \le 6$ forces $\min(A) \le 7$" to identify a small-min mult-indep triple in every certified $A$, and **all 21,338 small-min triples in $[3, 100]$ are empirically verified to close by Charge γ (64,014 verifications, 0 failures)**. Combined: every hypothesis-meeting $(A, k)$ in the certified scope is closed by at least one of three independent algebraic routes, **with no open transcendence-theory dependency**.
