...
To bridge the gap between contiguous interval absorption and the total sum, we introduce the concept of a *perfect seed*. Suppose computation provides an initial finite seed $S_0$ such that its subset sums cover a contiguous interval $[0, \Sigma(S_0)]$. By Lemma 3.1, if the next element $s_{N+1}$ satisfies $s_{N+1} \le U_N + 1$, the new contiguous interval upper bound becomes $U_{N+1} = U_N + s_{N+1}$. Since $U_N = \Sigma_N$, this implies $U_{N+1} = \Sigma_{N+1}$. Thus, the property $U_N = \Sigma_N$ is preserved inductively as long as absorption succeeds.
...
This establishes a profound hybrid closure mechanism. The general proof for bases $\ge 3$ necessitates a computational component:
